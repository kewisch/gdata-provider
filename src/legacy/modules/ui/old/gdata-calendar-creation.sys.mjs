...
export function gdataInitUI(window, document, version) {
  ...
  const { getGoogleSessionManager } = ChromeUtils.importESModule(
    `resource://gdata-provider/legacy/modules/old/gdataSession.sys.mjs?version=${version}`
  );
  ...
  (function() {
    ...
    let providerLabel = messenger.i18n.getMessage("gdataProviderLabel");
    ...
    window.registerCalendarType({
      ...
      label: messenger.i18n.getMessage("gdataProviderLabelLegacy", [providerLabel]),
    });
      ...
      window.MozXULElement.parseXULToFragment(`
        <vbox id="gdata-session" ${backLabel} ${findCalendarLabel}>
          <description>${messenger.i18n.getMessage(
            "gdata.wizard.session.legacy"
          )}</description>
          <hbox pack="end">
            <button id="gdata-report-issue" label="${messenger.i18n.getMessage("gdata.wizard.session.reportIssue")}"/>
          </hbox>
          <description>${messenger.i18n.getMessage(
    ...
    document.getElementById("gdata-report-issue").addEventListener("click", gdataReportIssue);
  })();
    ...
    Promise.allSettled([session.getTasksList(), session.getCalendarList()]).then(
      ([
        ...
        let calcals = calendars.map(calendarEntry => {
          ...
          let calendarName = calendarEntry.summaryOverride || calendarEntry.summary;
          calendar.name = messenger.i18n.getMessage("gdataProviderLabelLegacy", [calendarName]);
          calendar.id = cal.getUUID();
  ...
  function gdataReportIssue() {
    openLinkExternally("https://github.com/kewisch/gdata-provider/issues", { addToHistory: false });
  }
}
